...
public:
    ...
    void push(U &&value) {
        ...
        stacks.peek().push(forward<U>(value));
    }
...
int main() {
    ...
    for (int i = 0; i < 11; ++i) {
        stack1.push(i);
        cout << i << " is pushed into the stack " << stack1.size() << endl;
    }

    while (!stack1.isEmpty()) {
        size_t stackNo = stack1.size();
        cout << stack1.pop() << " is popped from the stack " << stackNo << endl;
    }
...
public:
    template<typename U>
    void push(U &&value) {
        stacks.push_back(forward<T>(value));
    }
